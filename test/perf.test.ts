...
describe("hook timing budgets", () => {
  it("starts fast: cold start p95 under the budget over a bare node process", () => {
    ...
    paired.sort((a, b) => a - b);
    const over = percentile(paired, 0.95);
    // eslint-disable-next-line no-console
    console.log(`hook cold start: median ${percentile(hook, 0.5).toFixed(1)} ms, p95 ${percentile(hook, 0.95).toFixed(1)} ms; node -e 0 median ${percentile(baseline, 0.5).toFixed(1)} ms; p95 over baseline ${over.toFixed(1)} ms (budget ${budget} ms)`);
    expect(over).toBeLessThan(budget);
  });
